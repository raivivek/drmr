...
def get_available_resource_managers():
    ...
    for name, rm in list(RESOURCE_MANAGERS.items()):
        if rm().is_installed():
...
def get_resource_manager(name):
    ...
    raise drmr.exceptions.ConfigurationError(
        'Unrecognized resource manager "{}"'.format(name)
    )
...
def guess_resource_manager():
    ...
    logger = logging.getLogger(
        "{}.{}".format(__name__, guess_resource_manager.__name__)
    )

    available_resource_managers = get_available_resource_managers()
    if available_resource_managers:
        logger.debug(
            "Available resource managers: {}".format(available_resource_managers)
        )
        if len(available_resource_managers) > 1:
            raise drmr.exceptions.ConfigurationError(
                "Multiple resource managers are available: {}".format(
                    available_resource_managers
                )
            )
        ...
        raise drmr.exceptions.ConfigurationError(
            "No recognized resource manager found."
        )
...
def load_configuration(config=None, file=None):
    ...
    if os.path.exists(file):
        ...
        raise drmr.exceptions.ConfigurationError(
            "Specified configuration file ({}) does not exist.".format(file)
        )
    ...
    if "resource_manager" not in config:
        try:
            ...
            logger.debug(
                """No resource manager configured, so I'm going with the one I found, {}.""".format(
